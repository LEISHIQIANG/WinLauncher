...
// ─────────────────────────────────────────────
static const float Y_SEC_BASIC  = 0.0f;    // "基本信息" separator
...
void ShortcutEditForm::EnsureFonts(IDWriteFactory* dwriteFactory)
{
                          ...
                          DWRITE_PARAGRAPH_ALIGNMENT va)
    {
        ...
        dwriteFactory->CreateTextFormat(L"Microsoft YaHei UI", nullptr,
...
{
    return HitTestRect(pt, D2D1::RectF(m_bounds.left + 20, m_bounds.top + Y_ADMIN_CB, m_bounds.left + 220, m_bounds.top + Y_ADMIN_CB + 22));
}
...
void ShortcutEditForm::DrawSectionLabel(ID2D1HwndRenderTarget* rt, const wchar_t* text, const D2D1_RECT_F& rect)
{
    ...
    if (sepBrush)
    {
        ...
        rt->DrawLine(D2D1::Point2F(rect.left, midY),
                     D2D1::Point2F(rect.right, midY), sepBrush.Get(), 0.5f);
    }
...
void ShortcutEditForm::DrawCheckbox(ID2D1HwndRenderTarget* rt, const D2D1_RECT_F& rect, bool checked, bool hovered)
{
    ...
    if (m_tfLabel)
    {
        ...
        const wchar_t* lblText = L"以管理员身份运行";
        if (tb) rt->DrawTextW(lblText, (UINT32)wcslen(lblText), m_tfLabel.Get(), labelRect, tb.Get());
    }
}
...
void ShortcutEditForm::Paint(ID2D1HwndRenderTarget* rt, float scale)
{
    ...
    if (m_tfLabel)
    {
        ...
        const wchar_t* lblText = L"名称  (最多6字)";
    ...
    if (m_tfLabel)
    {
        D2D1_COLOR_F lc = UIStyle::ThemeColor::TextNormal().d2d; lc.a = 0.75f;
        auto lb = GetOrCreateBrush(rt, lc);
        const wchar_t* lblText = L"自定义图标  (可选)";
        if (lb) rt->DrawTextW(lblText, (UINT32)wcslen(lblText), m_tfLabel.Get(),
    ...
    // Run as admin checkbox (left side)
    DrawCheckbox(rt, D2D1::RectF(m_bounds.left + 20, m_bounds.top + Y_ADMIN_CB, m_bounds.left + 220, m_bounds.top + Y_ADMIN_CB + 22), m_runAsAdmin, m_hoveredRunAsAdmin);
