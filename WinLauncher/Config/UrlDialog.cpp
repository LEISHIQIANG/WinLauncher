...
static const float Y_BUTTONS    = Y_FORM_TOP + UrlEditForm::PreferredContentHeight() + 20.0f;
static const int  DLG_H         = (int)(Y_BUTTONS + 45.0f);

static UrlDialog* g_udInstance = nullptr;

struct UrlDialogBrushCacheEntry
{
    D2D1_COLOR_F color;
    ComPtr<ID2D1SolidColorBrush> brush;
};
static std::vector<UrlDialogBrushCacheEntry> g_urlDialogBrushCache;

static ComPtr<ID2D1SolidColorBrush> GetOrCreateDialogBrush(ID2D1HwndRenderTarget* rt, const D2D1_COLOR_F& color)
{
    for (auto& entry : g_urlDialogBrushCache)
    {
    ...
    if (rt)
    {
        ...
        if (brush)
        {
            g_urlDialogBrushCache.push_back({ color, brush });
        }
    }
    return brush;
}

UrlDialog::UrlDialog(const wchar_t* title, const InitParams& init, AppContext* ctx)
    : m_title(title)
    , m_init(init)
    , m_okPressed(false)
{
...
bool UrlDialog::Show(HWND parent, const wchar_t* title,
                     UrlDialogResult& result,
                     const InitParams* init, AppContext* ctx)
{
    if (g_udInstance) return false;

    InitParams params{};
    if (init) params = *init;

    UrlDialog* win = new UrlDialog(title, params, ctx);
    ...
    if (ok)
    {
        UrlEditFormResult formRes = win->m_form.GetResult();
        result.name = formRes.name;
        result.url = formRes.url;
        result.browserPath = formRes.browserPath;
        result.browserArgs = formRes.browserArgs;
...
LRESULT UrlDialog::HandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
    case WM_CREATE:
...
void UrlDialog::OnPaintContent(ID2D1HwndRenderTarget* rt)
{
    EnsureFonts();
...
void UrlDialog::UpdateChildLayout()
{
    float scale = GetWindowScale(GetHWND());
    m_form.UpdateLayout(D2D1::RectF(0, Y_FORM_TOP, DLG_W, DLG_H), scale);
}

bool UrlDialog::HitTestRect(POINT pt, const D2D1_RECT_F& rect)
{
    return (pt.x >= rect.left && pt.x <= rect.right && pt.y >= rect.top && pt.y <= rect.bottom);
}

bool UrlDialog::HitTestCloseButton(POINT pt)
{
...
bool UrlDialog::HitTestOkButton(POINT pt)
{
    return HitTestRect(pt, D2D1::RectF(DLG_W - 175, Y_BUTTONS, DLG_W - 95, Y_BUTTONS + 26));
}

bool UrlDialog::HitTestCancelButton(POINT pt)
{
    return HitTestRect(pt, D2D1::RectF(DLG_W - 90, Y_BUTTONS, DLG_W - 20, Y_BUTTONS + 26));
}

void UrlDialog::DrawButton(ID2D1HwndRenderTarget* rt, const wchar_t* text,
