...
static const float Y_BUTTONS    = Y_FORM_TOP + CommandEditForm::PreferredContentHeight() + 20.0f;
static const int  DLG_H         = (int)(Y_BUTTONS + 45.0f);

static CommandDialog* g_cdInstance = nullptr;

struct CommandDialogBrushCacheEntry
{
    D2D1_COLOR_F color;
    ComPtr<ID2D1SolidColorBrush> brush;
};
static std::vector<CommandDialogBrushCacheEntry> g_commandDialogBrushCache;

static ComPtr<ID2D1SolidColorBrush> GetOrCreateDialogBrush(ID2D1HwndRenderTarget* rt, const D2D1_COLOR_F& color)
{
    for (auto& entry : g_commandDialogBrushCache)
    {
    ...
    if (rt)
    {
        ...
        if (brush)
        {
            g_commandDialogBrushCache.push_back({ color, brush });
        }
    }
    return brush;
}

CommandDialog::CommandDialog(const wchar_t* title, const InitParams& init, AppContext* ctx)
    : m_title(title)
    , m_init(init)
    , m_okPressed(false)
{
...
bool CommandDialog::Show(HWND parent, const wchar_t* title,
                         CommandDialogResult& result,
                         const InitParams* init, AppContext* ctx)
{
    if (g_cdInstance) return false;

    InitParams params{};
    if (init) params = *init;

    CommandDialog* win = new CommandDialog(title, params, ctx);
    ...
    if (ok)
    {
        CommandEditFormResult formRes = win->m_form.GetResult();
        result.name = formRes.name;
        result.command = formRes.command;
        ...
        result.maxChars = formRes.maxChars;
    }

    g_cdInstance = nullptr;
    delete win;
    ...
    g_commandDialogBrushCache.clear();
    return ok;
}

LRESULT CommandDialog::HandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
    case WM_CREATE:
...
void CommandDialog::OnPaintContent(ID2D1HwndRenderTarget* rt)
{
    EnsureFonts();
...
void CommandDialog::EnsureFonts()
{
    if (m_tfTitle) return;
...
void CommandDialog::UpdateChildLayout()
{
    float scale = GetWindowScale(GetHWND());
    m_form.UpdateLayout(D2D1::RectF(0, Y_FORM_TOP, DLG_W, DLG_H), scale);
}

bool CommandDialog::HitTestRect(POINT pt, const D2D1_RECT_F& rect)
{
    return (pt.x >= rect.left && pt.x <= rect.right && pt.y >= rect.top && pt.y <= rect.bottom);
}

bool CommandDialog::HitTestCloseButton(POINT pt)
{
...
bool CommandDialog::HitTestOkButton(POINT pt)
{
    return HitTestRect(pt, D2D1::RectF(DLG_W - 175, Y_BUTTONS, DLG_W - 95, Y_BUTTONS + 26));
}

bool CommandDialog::HitTestCancelButton(POINT pt)
{
    return HitTestRect(pt, D2D1::RectF(DLG_W - 90, Y_BUTTONS, DLG_W - 20, Y_BUTTONS + 26));
}

void CommandDialog::DrawButton(ID2D1HwndRenderTarget* rt, const wchar_t* text,
