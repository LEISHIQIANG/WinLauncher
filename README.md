...
> 版本 **0.5.1.6** | C++17 | MSVC v143 | Windows 10+
...
> Version **0.5.1.6** | C++17 | MSVC v143 | Windows 10+
...
main.cpp  ──►  Application  ──►  AppContext (DI Container)
   ...
   └─ AppearanceSettings     ├─ IIconService                └─ ConfigViewModel
                             ...
                             └─ PrivilegeLaunchService   UI Layer   Render
                                                         ├─ Popup   ├─ Compositor
UI Window Hierarchy                                     ├─ Config  ├─ BgLayer
BaseWindow ──► GlassWindow ──► PopupWindow               └─ Tray    └─ Overlay
                    │                                      Menu
                    ├─► ConfigWindow
                    └─► TrayMenuWindow
...
| **Event-Driven** | `EventBus` with typed pub/sub for `ConfigChanged`, `ShortcutLaunched`, `PopupShown`, `PopupHidden`, `ThemeChanged`, `UiScaleChanged`, `AppQuit` |
...
\-- WinLauncher/
    ...
    +-- version.h                         # Version macros (0.5.1.6)
